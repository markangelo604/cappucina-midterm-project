<?php
require_once 'booking-config.php';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?php echo $page_title; ?></title>
    <link rel="stylesheet" href="css/booking.css">
</head>
<body>
    
    <!-- Navigation Bar -->
    <nav class="navbar">
        <div class="container">
            <div class="nav-content">
                <div class="logo">
                    <img src="images/merrylift-transparent.png" alt="Logo">
                </div>
                <ul class="nav-menu">
                    <?php foreach ($nav_items as $items): ?>
                        <li>
                            <a href="<?php echo $items['url']; ?>" 
                            class="<?php echo $items['active'] ? 'active' : ''; ?>">
                            <?php echo $items['name']; ?>
                            </a>
                        </li>
                    <?php endforeach; ?>
                </ul>
                <div class="nav-buttons">
                    <button class="btn-Outline" onclick="window.location.href='html/login.html'">Sign In</button>
                    <button class="btn-primary" onclick="window.location.href='html/signup.html'">Join Now</button>
                </div>
            </div>
        </div>
    </nav>

    <!-- Booking Section -->
    <section class="booking-section">
        <div class="container">
            <div class="booking-header">
                <h1>My Bookings</h1>
                <p>View and manage your ride bookings</p>
            </div>

            <!-- Filter Tabs -->
            <div class="filter-container">
                <span class="filter-label">Filter by:</span>
                <div class="filter-tabs">
                    <button class="tab-btn active" data-filter="all">All</button>
                    <button class="tab-btn" data-filter="upcoming">Upcoming</button>
                    <button class="tab-btn" data-filter="completed">Completed</button>
                    <button class="tab-btn" data-filter="cancelled">Cancelled</button>
                </div>
            </div>

            <!-- Booking Cards -->
            <div class="bookings-list">
                <?php foreach ($bookings as $booking): ?>
                <div class="booking-card" data-status="<?php echo strtolower($booking['status']); ?>">
                    <div class="card-left-border status-<?php echo $booking['status_class']; ?>"></div>
                    
                    <div class="booking-main">
                        <div class="booking-header-row">
                            <div class="status-badge status-<?php echo $booking['status_class']; ?>">
                                <?php echo $booking['status']; ?>
                            </div>
                            <div class="booking-id-price">
                                <span class="booking-id">Booking <?php echo $booking['id']; ?></span>
                                <span class="booking-price"><?php echo $booking['price']; ?></span>
                            </div>
                        </div>

                        <div class="booking-body">
                            <div class="route-section">
                                <div class="route-item">
                                    <svg width="12" height="12" viewBox="0 0 12 12" class="route-icon">
                                        <circle cx="6" cy="6" r="4" fill="#E74C3C"/>
                                    </svg>
                                    <span class="route-text"><?php echo $booking['pickup']; ?></span>
                                </div>
                                <div class="route-item">
                                    <svg width="12" height="12" viewBox="0 0 12 12" class="route-icon">
                                        <circle cx="6" cy="6" r="4" fill="#9B59B6"/>
                                    </svg>
                                    <span class="route-text"><?php echo $booking['destination']; ?></span>
                                </div>
                            </div>

                            <div class="booking-details">
                                <div class="detail-item">
                                    <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" class="detail-icon">
                                        <circle cx="8" cy="8" r="7" fill="none" stroke="currentColor" stroke-width="1.5"/>
                                        <path d="M8 4v4l3 3" stroke="currentColor" stroke-width="1.5" fill="none"/>
                                    </svg>
                                    <span><?php echo $booking['date']; ?></span>
                                </div>
                                <div class="detail-item">
                                    <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" class="detail-icon">
                                        <path d="M8 5c-1.7 0-3 1.3-3 3s1.3 3 3 3 3-1.3 3-3-1.3-3-3-3z"/>
                                        <circle cx="8" cy="8" r="2" fill="white"/>
                                    </svg>
                                    <span><?php echo $booking['passengers']; ?></span>
                                </div>
                                <div class="detail-item">
                                    <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" class="detail-icon">
                                        <rect x="2" y="4" width="12" height="8" rx="1" fill="none" stroke="currentColor" stroke-width="1.5"/>
                                        <line x1="2" y1="8" x2="14" y2="8" stroke="currentColor" stroke-width="1.5"/>
                                    </svg>
                                    <span><?php echo $booking['payment_status']; ?></span>
                                </div>
                                <?php if (isset($booking['cancel_reason'])): ?>
                                <div class="detail-item cancel-reason">
                                    <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" class="detail-icon">
                                        <path d="M8 0l1.5 5h5l-4 3.5 1.5 5-4-3-4 3 1.5-5-4-3.5h5z" fill="#FFA500"/>
                                    </svg>
                                    <span><?php echo $booking['cancel_reason']; ?></span>
                                </div>
                                <div class="detail-item refund-badge">
                                    <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" class="detail-icon">
                                        <circle cx="8" cy="8" r="7" fill="#FFA500"/>
                                        <text x="8" y="11" text-anchor="middle" fill="white" font-size="10" font-weight="bold">₱</text>
                                    </svg>
                                    <span>Refunded</span>
                                </div>
                                <?php endif; ?>
                            </div>
                        </div>

                        <div class="booking-footer">
                            <div class="driver-info">
                                <div class="driver-avatar" style="background-color: <?php echo $booking['driver_color']; ?>">
                                    <?php echo $booking['driver_initials']; ?>
                                </div>
                                <div class="driver-details">
                                    <h4><?php echo $booking['driver_name']; ?></h4>
                                    <div class="driver-rating">
                                        <span class="star-icon">★</span>
                                        <span><?php echo $booking['rating']; ?></span>
                                        <span class="rating-count"><?php echo $booking['total_ratings']; ?></span>
                                    </div>
                                </div>
                            </div>

                            <div class="action-buttons">
                                <?php if ($booking['status'] === 'Confirmed'): ?>
                                    <button class="btn-secondary">View Details</button>
                                    <button class="btn-danger">Cancel Booking</button>
                                <?php elseif ($booking['status'] === 'Completed'): ?>
                                    <button class="btn-secondary">View Details</button>
                                    <button class="btn-primary-action">Rate Driver</button>
                                <?php else: ?>
                                    <button class="btn-secondary">View Details</button>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
    </section>

    <!-- Footer -->
    <footer class="footer">
        <div class="container">
            <div class="footer-content">
                <div class="footer-brand">
                    <h3>MerryLift</h3>
                    <p>Your trusted carpooling platform in the Philippines. Share rides, save money, and build community through travel.</p>
                    <div class="footer-social">
                        <div class="social-icon">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                            </svg>
                        </div>
                        <div class="social-icon">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                            </svg>
                        </div>
                        <div class="social-icon">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.896-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678c-3.405 0-6.162 2.76-6.162 6.162 0 3.405 2.76 6.162 6.162 6.162 3.405 0 6.162-2.76 6.162-6.162 0-3.405-2.76-6.162-6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405c0 .795-.646 1.44-1.44 1.44-.795 0-1.44-.646-1.44-1.44 0-.794.646-1.439 1.44-1.439.793-.001 1.44.645 1.44 1.439z"/>
                            </svg>
                        </div>
                    </div>
                </div>
                <?php foreach ($footer_links as $category => $links): ?>
                    <div class="footer-links">
                        <h4><?php echo $category; ?></h4>
                        <ul>
                            <?php foreach ($links as $link): ?>
                                <li><a href="#"><?php echo $link; ?></a></li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
                <?php endforeach; ?>
            </div>
            <div class="footer-bottom">
                <p>&copy; <?php echo date("Y"); ?> MerryLift. All Rights Reserved.</p>
            </div>
        </div>
    </footer>

    <script>
        // Filter functionality
        document.querySelectorAll('.tab-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
                this.classList.add('active');
                
                const filter = this.dataset.filter;
                const cards = document.querySelectorAll('.booking-card');
                
                cards.forEach(card => {
                    if (filter === 'all' || card.dataset.status === filter) {
                        card.style.display = 'block';
                    } else {
                        card.style.display = 'none';
                    }
                });
            });
        });
    </script>

</body>
</html>